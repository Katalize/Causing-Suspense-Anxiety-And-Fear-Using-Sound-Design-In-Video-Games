...
public class FMODFtstps : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (DistanceTraveled >= StepDistance)
        {
            ...
            GroundCollision();
            MaterialCheck();
            Debug.Log ("The material value is set to " + FMOD_F_Material);
            SpeedCheck();
            Debug.Log("Running is set to " + FMOD_F_Running);
            PlayFootstep();
    ...
    void MaterialCheck()
    {
        if (Physics.Raycast(transform.position, Vector3.down, out hit, RayDistance))
        {
            ...
            else if (hit.transform.tag == "Area1")
            {
             ...
             else if (hit.transform.tag == "Area2")
            {
                FMOD_F_Material = 2;
            }
              else if (hit.transform.tag == "Area3")
            {
                FMOD_F_Material = 3;
            }
            else
